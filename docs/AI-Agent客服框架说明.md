...
| 模块 | 职责 | 当前状态 |
|------|------|----------|
| smartcs-gateway | 用户消息入口、鉴权限流、会话管理、消息标准化 | 已建骨架 |
| smartcs-agent-core | NLU、对话管理、槽位填充、风控路由、人工兜底决策 | 已建骨架 |
| smartcs-skill-engine | 技能注册、技能路由、声明式 API 编排 | 已建骨架 |
| smartcs-workbench | 人工审核、工单流转、坐席接管、审批审计 | 已建骨架 |
| smartcs-knowledge | FAQ、政策知识库、RAG 检索增强 | 已建骨架 |
| smartcs-notification | 用户通知、坐席提醒、事件通知 | 已建骨架 |
| smartcs-common | 跨服务契约、DTO、枚举、错误码、工具 | 已建骨架 |
| workstation | 坐席工作台前端 | 已建目录 |
| client-sdk/h5 | 用户端 H5 聊天组件 | 已建目录 |
...
1. 在 common 中定义跨模块契约：标准消息、意图结果、槽位、风险决策、工单事件。
2. 在 gateway 中接入最小 WebSocket / REST 入口和会话状态。
3. 在 agent-core 中实现 NLU 结果结构、对话状态机和风险路由接口。
4. 在 skill-engine 中实现技能注册表、技能路由和声明式执行框架。
5. 在 workbench 中实现 L3 工单创建、审批、接管和审计。
6. 在 knowledge 中接入 FAQ / RAG，只服务知识问答，不承载实时订单数据。
