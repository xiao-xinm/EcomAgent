...
当前结论：**Phase 7 暂不立即实现 WebSocket**。用户端 SSE 最小试点已经完成；下一步将同样的“事件只做增量提示、完整数据仍走查询接口”原则扩展到坐席端工单提醒。WebSocket 留给后续真正双向人工聊天或多坐席协作。
...
- 工单列表和详情尚未自动感知其他页面、其他坐席或 Agent Core 创建的工单变化。
...
- `stream.ready`：连接建立后触发一次完整刷新。
- `ticket.changed`：工单或关联坐席动作发生变化。
- `heartbeat`：保持连接活跃。

实现原则：

- SSE 只提示 `ticketId`、当前状态、坐席和变化时间，不在事件中复制完整工单。
- 列表页收到事件后合并刷新工单列表和统计。
- 详情页只在事件 `ticketId` 与当前工单一致时刷新详情、消息和操作日志。
- 后端从共享 MySQL 的 `work_order` 与 `audit_log` 增量扫描变化，多实例不依赖进程内广播。
- 后端和前端开关都默认关闭，失败时保留手动刷新及操作后刷新。
- 原生 `EventSource` 无法携带 Bearer Header；严格鉴权环境暂不开启，待真实账号中心明确 Cookie/BFF 或长连接票据方案后再接入。
...
- 保留当前操作后刷新详情。
- 可配置开启工单 SSE 提醒，收到事件后复用现有查询接口刷新列表、详情、消息和操作日志。
- 不通过 SSE 发送坐席操作命令，也不在本轮实现在线状态、输入中或已读回执。
...
- 坐席列表页打开 SSE 后，新建工单或工单状态变化能触发列表与统计刷新。
- 坐席详情页只响应当前 `ticketId` 的变化，其他工单事件不触发详情刷新。
- 内部备注和人工消息等不改变工单状态的动作，也能触发当前详情刷新。
...
- 保持用户端短轮询、坐席端手动刷新和操作后刷新作为稳定基线。
- 用户端与坐席端 SSE 都保持默认关闭，先用于本地和测试环境验证。
