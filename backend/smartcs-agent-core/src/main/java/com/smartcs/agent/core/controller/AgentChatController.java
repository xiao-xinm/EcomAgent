...
public class AgentChatController {
    ...
    public ApiResponse<AgentReply> chat(@Valid @RequestBody ChatRequest request) {
        LOGGER.info(
                "Agent Core收到聊天请求 {} channel={}",
                LogFields.chat(request.traceId(), request.sessionId(), request.userId()),
                request.channel());
        AgentReply reply = mockAgentOrchestrator.process(request);
        LOGGER.info(
                "Agent Core生成回复 {} replyId={} routeDecision={} riskLevel={} ticketId={}",
                LogFields.chat(reply.traceId(), reply.sessionId(), request.userId()),
                reply.replyId(),
                reply.routeDecision(),
                reply.riskLevel(),
    ...
    public ApiResponse<AgentReply> action(@Valid @RequestBody ChatActionRequest request) {
        LOGGER.info(
                "Agent Core收到聊天动作 {} actionType={}",
                LogFields.chat(request.traceId(), request.sessionId(), request.userId()),
                request.actionType());
        AgentReply reply = mockAgentOrchestrator.processAction(request);
        LOGGER.info(
                "Agent Core生成动作回复 {} replyId={} routeDecision={} riskLevel={}",
                LogFields.chat(reply.traceId(), reply.sessionId(), request.userId()),
                reply.replyId(),
                reply.routeDecision(),
                reply.riskLevel());
